...
class QATrainer:
	def __init__(self, config):
		...
		self.tokenizer = configmapper.get_object("tokenizers", self.config.train.tokenizer.name).from_pretrained(**self.config.train.tokenizer.init_params.as_dict())

	def train(self, model,train_dataset, val_dataset=None):
		args = TrainingArguments(**self.train_config.trainer_args.as_dict())
		data_collator = default_data_collator
		trainer = Trainer(model,args,train_dataset=train_dataset,eval_dataset=val_dataset,data_collator=data_collator,tokenizer=tokenizer)
		trainer.train()
